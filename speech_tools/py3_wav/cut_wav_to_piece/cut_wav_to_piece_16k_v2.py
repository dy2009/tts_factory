...
g_vad_frame_len_ms = 700


global g_step_frame_len_ms
g_step_frame_len_ms = 5
...
global g_step_frame_len
g_step_frame_len = g_step_frame_len_ms * int(g_sample_rate / 1000)


print("g_step_frame_len=",g_step_frame_len)
print("g_vad_frame_len=",g_vad_frame_len)


# 10ms * 
global g_sil_energy_threshold
g_sil_energy_threshold = g_vad_frame_len * 10
...
def cut_wav_sil(cur_file_path,out_dir,bflag_add_sil):
    global g_sample_rate
    global g_bit_depth
    #
    global g_index
    ...
    while(True):
        ...
        if(not bflag_have_wav):
            ...
            if(cur_buf_e < g_sil_energy_threshold):
                ...
                wav_piect_i = in_wav2[start_index:i_step + g_vad_frame_len]

                if(bflag_add_sil):
                    # cut
                    head_sil = np.random.randint(-10,10,sil_90ms) / math.pow(2,g_bit_depth -1)
                    tail_sil = np.random.randint(-10,10,sil_90ms) / math.pow(2,g_bit_depth -1)
                    #
                    wav_piect_i = np.hstack((head_sil, wav_piect_i, tail_sil))
                #
                dst_name = "{0:06d}".format(g_index) + ".wav"
...
def wav_to_wav16k(in_dir,out_dir,bflag_add_sil):
    global g_suffix_del
    global g_index
    #
    ...
    for i in tqdm(range(len(file_list))):
        ...
        #
        cut_wav_sil(cur_file_path,out_dir,bflag_add_sil)
        #
...
if __name__ == "__main__":
    ...
    #
    wav_to_wav16k(in_dir,out_dir,bflag_add_sil)
