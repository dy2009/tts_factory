...
g_win_point_num = int( g_win_ms_len * (g_sample_rate // 1000))
g_hop_point_num = int( g_hop_ms_len * (g_sample_rate // 1000))
...
def step9_del_wav16k_long_sil_1wav(in_wav_path):
    global g_win_point_num
    global g_hop_point_num
    global g_sil_point_value_int
    global g_sil_point_value_f
    ...
    while(True):
        ...
        #
        if(len(cur_buf) < g_win_point_num):
            break
        #
    ...
    # print("cur_wav=",in_wav_path,",sil_frame_rate=",sil_frame_rate)

    if(sil_frame_rate >= 0.9):
        return True
    else:
        return False
...
def step9_del_long_sil_wav_txt(in_txt,out_txt):
    ...
    for curLine in cur_f.readlines():
        ...
        if(len(line_cp) <= 0):
            continue
        #
        bflag_is_sil = step9_del_wav16k_long_sil_1wav(line_cp)
        #
        if(bflag_is_sil):
            os.remove(line_cp)
        else:
...
def step9_del_long_sil_wav_dir(in_wav_dir):
    #
    ...
    sum_num = 0
    #
    for i in tqdm(range(len(in_wav_list))):
        cur_wav = in_wav_list[i]
        cur_wav_path = os.path.join(in_wav_dir,cur_wav)
        #
        #
        bflag_is_sil = step9_del_wav16k_long_sil_1wav(cur_wav_path)
        #
        sum_num += 1

        if(bflag_is_sil):
            del_num += 1
            print("sum_num=",sum_num,",del_num=",del_num)
            # print(cur_wav_path)
...
if __name__ == "__main__":
    ...
    #
    # in_wav_dir = "D:\\t_0003\\wav16k"
    #
    step9_del_long_sil_wav_dir(in_wav_dir)
