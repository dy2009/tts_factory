...
def step4_cut_piece_wav24k_c1_in_wav_txt_out_dir(in_wav_txt, out_wav_dir):
    # head_sil_point_num = int(90 * 16)

    # in_wav_list = os.listdir(in_wav_dir)
    # in_wav_list.sort()

    in_wav_list = read_txt_to_list(in_wav_txt)

    for w_index in range(len(in_wav_list)):
        in_wav_path = in_wav_list[w_index]
        #
        cur_w_name = os.path.basename(in_wav_path)
        if(cur_w_name[-4:] != ".wav"):
            continue
...
if __name__ == "__main__":
    ...
    #
    step4_cut_piece_wav24k_c1_in_wav_txt_out_dir(in_wav_txt, out_wav_dir)
